...
class User:
    ...
    @classmethod
    def get_users_trip(cls,data):
        query = "select * from users left join trips on trips.user_id = users.id left join join_trip on join_trip.user_id=users.id where users.id != %(user_id)s and join_trip.trip_id=%(trip_id)s group by users.id;"
        results = connectToMySQL(DATABASE).query_db(query,data)
        users = []
        for user in results:
            users.append( cls(user) )
